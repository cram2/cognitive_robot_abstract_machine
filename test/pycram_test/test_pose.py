...
def test_pose_creation(immutable_model_world):
    ...
    p = Pose.from_list([1, 2, 3], [0, 0, 0, 1], world.root)
...
def test_pose_to_transform(immutable_model_world):
    world, robot_view, context = immutable_model_world
    p = Pose.from_list([3, 2, 1], [0, 0, 1, 0], world.root)

    transform = p.to_transform_stamped(world.get_body_by_name("r_gripper_tool_frame"))

    assert transform == HomogeneousTransformationMatrix.from_list(
        [3, 2, 1],
        [0, 0, 1, 0],
        world.root,
...
def test_pose_edit(immutable_model_world):
    world, robot_view, context = immutable_model_world
    p = Pose.from_list([3, 4, 5], [0, 1, 0, 0], world.root)
...
def test_pose_copy(immutable_model_world):
    world, robot_view, context = immutable_model_world
    p1 = Pose.from_list([1, 2, 3], [0, 0, 0, 1], world.root)
    p2 = deepcopy(p1)
...
def test_transform_creation(immutable_model_world):
    ...
    t = HomogeneousTransformationMatrix.from_list(
        [1, 2, 3], [0, 0, 0, 1], world.root, test_body
    )
...
def test_transform_edit(immutable_model_world):
    ...
    test_body = world.get_body_by_name("milk.stl")
    t = HomogeneousTransformationMatrix.from_list(
        [3, 2, 1], [0, 1, 0, 0], world.root, test_body
    )
...
def test_transform_copy(immutable_model_world):
    ...
    test_body = world.get_body_by_name("milk.stl")
    t = HomogeneousTransformationMatrix.from_list(
        [1, 1, 1], [0, 0, 0, 1], world.root, test_body
    )
...
def test_transform_multiplication(immutable_model_world):
    ...
    t2 = HomogeneousTransformationMatrix.from_list(
        [3, 2, 1],
        [0, 0, 0, 1],
        test_body,
        world.get_body_by_name("r_gripper_tool_frame"),
...
def test_is_facing_2d_axis(immutable_model_world):
    world, robot_view, context = immutable_model_world
    a = Pose.from_list([0, 0, 0], [0, 0, 0, 1], world.root)  # facing +x
    b = Pose.from_list([1, 0, 0], [0, 0, 0, 1], world.root)

    facing, angle = a.is_facing_2d_axis(b, axis=Vector3.X())
    assert facing
    assert angle == pytest.approx(0, abs=1e-6)

    # now krrood_test Y alignment (should be 90 deg difference)
    facing_y, angle_y = a.is_facing_2d_axis(b, axis=Vector3.Y())
    assert not facing_y
...
def test_is_facing_x_or_y(immutable_model_world):
    world, robot_view, context = immutable_model_world
    a = Pose.from_list([0, 0, 0], [0, 0, 0, 1], world.root)
    b = Pose.from_list([1, 0, 0], [0, 0, 0, 1], world.root)
...
def test_transform_stamped_multiplication(immutable_model_world):
    world, robot_view, context = immutable_model_world
    t1 = HomogeneousTransformationMatrix.from_list(
...
def test_transform_inverse(immutable_model_world):
    world, robot_view, context = immutable_model_world
    p = Pose.from_list([1, 2, 3], [0, 0, 0, 1], robot_view.root)
...
def test_inverse_matrix():
    p = Pose.from_list([3.1, 2, 0], [0, 0, 0, 1])
