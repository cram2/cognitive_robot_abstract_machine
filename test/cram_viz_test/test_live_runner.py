"""
Unit tests for :func:`cram_viz.live.runner.start`'s control flow.

``hooks.install_*`` monkey-patches coraplex/giskardpy classes process-globally with no
uninstall (see :meth:`cram_viz.live.bridge.Bridge.claim_hook`), so these tests replace
them with no-ops rather than calling them for real, and substitute a fresh
:class:`Bridge` for :data:`cram_viz.live.runner.BRIDGE` so no test touches or dirties
...
from cram_viz.live import runner
