    ...
    with world.modify_world():
        ...
        world.add_connection(
            FixedConnection(
                parent=world.root,
                child=target,
                parent_T_connection_expression=HomogeneousTransformationMatrix.from_xyz_rpy(x=1.0)
            )
        )
    ...
    with world.modify_world():
        ...
        world.add_connection(
            FixedConnection(
                parent=world.root,
                child=obstacle,
                parent_T_connection_expression=HomogeneousTransformationMatrix.from_xyz_rpy(x=0.5, y=0.5)
            )
        )
    return world, target
...
def test_spawn_as_region(simple_world):
    ...
    expected_center_x = (box.min_x + box.max_x) / 2
    expected_center_y = (box.min_y + box.max_y) / 2
    expected_center_z = (box.min_z + box.max_z) / 2
