"""
Tests for verbalizing EQL *match* expressions (``match`` / ``underspecified`` /
``match_variable``).

A match constructed via ``match`` or ``underspecified`` is a generative request →
*"Generate"*; a domain search (a ``MatchVariable``) → *"Find"*.  A match has two
condition parts: the construction-pattern equalities (the ``kwargs``) → *"given that"*,
and the ``.where(...)`` conditions → *"where"*.  Each condition is its own point;
equality assignments on the same object are grouped (*"x, y, and z of the Position are
1, 2, and 3 respectively"*); an ``Ellipsis`` value is a value to generate → folded into
the header (*"… and predict its x, y, and z values"*).
"""
