...
class SoftPCCConnection(Connection):
    ...
    def __init__(self, parent, child, kappa_dof, phi_dof, length: float, name=None):
        ...
        # Position formulas
        px = cs.if_else(
            it_is_straight, 0, (cs.cos(p) * (1 - cs.cos(theta))) / (k + 1e-12)
        )
        py = cs.if_else(
            it_is_straight, 0, (cs.sin(p) * (1 - cs.cos(theta))) / (k + 1e-12)
        )
...
class CosseratRodConnection(Connection):
    ...
    def __init__(
        self, parent, child, ux_dof, uy_dof, uz_dof, vz_dof, length: float, name=None
    ):
        self.segment_length = length
