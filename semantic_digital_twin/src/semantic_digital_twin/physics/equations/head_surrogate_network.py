...
class HeadSurrogate(torch.nn.Module):
    ...
    Input normalization is baked into :meth:`forward`, so the network drops into the symbolic
    pipeline unchanged. Trained with Sobolev (value + gradient) supervision against the analytic
    head, so the gradients the MPC linearizes are accurate, not just the values.
    """
