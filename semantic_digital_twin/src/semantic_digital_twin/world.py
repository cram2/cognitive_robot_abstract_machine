...
class World(HasSimulatorProperties):
    ...
    def set_dofs_allow_external_state_update(
        ...
        """
        Sets whether the specified degrees of freedom may be updated from an external source.

        Call this inside a :meth:`modify_world` block so the change is tracked and synchronized to
        other processes. Whoever adds the object declares this, since the object (and its DOFs) does
        not exist at server setup.

        :param dofs: The degrees of freedom whose ``allows_external_state_update`` flag is set.
        :param value: Whether the DOFs may be updated externally (``True``) or not (``False``).
        """
        for dof in dofs:
